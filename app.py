...
@st.dialog("📖 核心三大維度說明", width="large")
def show_strategy_guide():
    st.markdown("""
    ### 🛡️ 維度 A：基本面體質評分 (Fundamental Score)
    *這是系統的第一道防線。*
    - **定義**：根據財報關鍵指標（獲利性、穩定性、成長性）算出的 0-100 分。
    - **意義**：決定這家公司「值不值得買」。分數越高，代表財報品質與獲利結構越優異。

    ---

    ### 📊 維度 B：市場情緒位階 (Market Consensus Bands)
    *這是基於**「歷史 PE」與「前瞻 EPS」**的相對估值，反映群眾的心理狀態。*
    - **便宜價 (Floor)**：群眾極度悲觀或忽視的底部，通常是絕佳的撿便宜時機。
    - **合理價 (Consensus)**：市場多數人認同的公道價值，股價在此處最為平穩。
    - **昂貴價 (Euphoria)**：群眾情緒亢奮、夢想溢價的頂點，風險報酬比開始惡化。

    ---

    ### ⚓ 維度 C：企業實力基石 (Intrinsic Value Base)
    *這是基於 ROE 複利公式 的絕對估值，反映公司的內在底氣。*
    
    $$
    \\text{內在價值} = \\text{淨值} \\times (1 + (\\text{ROE} - \\text{10Y Bond Yield}))^{10}
    $$
    
    - **意義**：這是公司的「地心引力」。股價受情緒影響會上下擺盪，但最終都會回頭尋找這條線。它是判斷長期「安全邊際」最重要的防線。
    """)
    if st.button("我瞭解了"):
        st.rerun()
